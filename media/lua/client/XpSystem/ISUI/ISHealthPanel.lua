...
function ISNewHealthPanel:instantiate()
    ...
    playerCharacterObj = self.character;
...
function ISNewHealthPanel:onClick(button)
    ...
    if button.internal == "PILLINFO" then
...
--    self.bodyPartPanel:setColorScheme(self.colorScheme);
    ...
    self.fitness = ISButton:new(self.healthPanel.x + 165, self.healthPanel.y, 70, 20, getText("ContextMenu_Fitness"), self, ISNewHealthPanel.onClick);
...
--    print "instant";
    ...
    self.pillEffectWindow = self.fitness;    
    self.pillEffectWindow = ISButton:new(self.healthPanel.x + 165 + 75, self.healthPanel.y, 90, 20, getText("IGUI_PillInfoOpen"), self, ISNewHealthPanel.onClick);
    self.pillEffectWindow.internal = "PILLINFO"
    self.pillEffectWindow.anchorTop = false
    self.pillEffectWindow.anchorBottom = true
    self.pillEffectWindow:initialise();
    self.pillEffectWindow:instantiate();
    self:addChild(self.pillEffectWindow);
    self.blockingAlpha = 0.0;
--    print "instant";
...
function ISHealthPanel:render()
    ...
    if self.otherPlayer then
        self.fitness:setVisible(false);
        self.pillEffectWindow:setVisible(false);
    end
--    self.healthPanel:render();
    ...
    self.fitness:setY(y);    
    self.pillEffectWindow:setY(y);

    --print(type(self.fitness) .. " --- " .. type(self.pillEffectWindow));
    ...
    local effect = doctor:getBetaEffect();
    local delta = doctor:getBetaDelta();
    --print("Beta effect: " .. tostring(effect) .. " | Beta delta: " .. tostring(delta));
    ...
    if doctor:getJoypadBind() ~= -1 then
        ...
        --self:drawText("Beta Pill Time Remaining: " .. tostring(effect), 8, self.height - PAD_BOTTOM - fontHgt * 2, 1, 1, 1, 1, UIFont.Small);
...
function ISHealthPanel:onGainJoypadFocus(joypadData)
    ...
    if self.otherPlayer then
        ...
        self:setISButtonForY(self.pillEffectWindow)
    end
end
...
function ISHealthPanel:onJoypadDown(button)
    ...
    if button == Joypad.YButton then
        ...
        self.pillEffectWindow:forceClick()
    end
end
